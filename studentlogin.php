<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Document</title>
  <link href="css/studentlogin.css" rel="stylesheet">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
</head>

<body>



  <div class="container-fluid main image">

    <div class="row justify-content-center align-items-center pagemain">
      <div class="col-11 col-sm-8 col-md-6 col-lg-6 col-xl-5 col-xxl-4 p-5 sloginpage">
        <div class="round "></div>

        <div class="sloginhead">
          <h2> LOGIN</h2>
        </div>

        <form>
          <div class="row mb-3 mt-5 ms-auto me-auto">
            <label for="inputEmail4" class="form-label">Class</label>
            <div class="col-sm-10">
              <input type="text" placeholder="" class="form-control" id="inputEmail3">
            </div>
          </div>
          <div class="row mb-3 ms-auto me-auto">
            <label for="inputPassword4" class="form-label">Subject</label>
            <div class="col-sm-10">
              <input type="text" placeholder="" class="form-control" id="inputPassword3">
            </div>
          </div>

          <div class="d-grid gap-2 col-6 mx-auto">
            <button class="btn btn-primary mt-5 signinbutton" type="button">Login</button>
          </div>
        </form>

      </div>
    </div>
  </div>



  </div>

  <script src="https://kit.fontawesome.com/64fc7c3650.js" crossorigin="anonymous"></script>
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>

</body>

</html>